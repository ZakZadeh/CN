...
def vid2jpg(inPath, outPath):
    for fileName in os.listdir(inPath):
        if ('.mp4' not in fileName):
            continue

        name, ext = os.path.splitext(fileName)
        outDirPath = os.path.join(outPath, name)
        videoPath = os.path.join(inPath, fileName)

        try:
            if os.path.exists(outDirPath):
                if not os.path.exists(os.path.join(outDirPath, 'image_00001.jpg')):
                    subprocess.call('rm -r {}'.format(outDirPath), shell=True)
                    print('remove {}'.format(outDirPath))
                    os.mkdir(outDirPath)
                else:
                    continue
            else:
        ...
        cmd = 'ffmpeg -i {} -vf scale=-1:1080 {}/image_%05d.jpg'.format(videoPath, outDirPath)
        print(cmd)
...
class VideoFolder(data.Dataset):
    ...
    def __init__(self, video_root, video_ext, nframes, loader = videoLoader, transform=None):
        video_names, video_idx = find_videos(video_root)
