...
class Custom3D(nn.Module):
    def __init__(self, params):
        ...
        self.nClass = params.nClass
        self.conv1 = nn.Sequential(
            ## input is (nc) x nT x 64 x 64
            nn.Conv3d(self.nc, self.ndf, (4,4,4), (1,2,2), (2,1,1), bias = False),
            nn.ReLU(True),
            ## state size. (ndf) x 32 x 32
        )
        self.conv2 = nn.Sequential(
        ...
        self.conv5 = nn.Sequential(
            nn.Conv3d(self.ndf * 8, self.ndf, 4, 1, 0, bias = False),
            nn.ReLU(True),
        )
...
class ResNet3D(nn.Module):
    def __init__(self, params):
        ...
        self.nClass = params.nClass
        self.resnet = nn.Sequential(
            torchvision.models.video.r3d_18(pretrained=False, progress=True),
            nn.ReLU(True)
        )
        # net = nn.Sequential(*list(net.children())[:2])
        self.fc1 = nn.Sequential(
